...
#ifndef TINYSPEECH_INT8_MULTICORE
#define TINYSPEECH_INT8_MULTICORE 0
#endif
...
static void conv3x3_pool2x2_requant_relu_to_padded_hwc_c_rvv_uk48x24(const int8_t *pad,
    ...
    for (int32_t ph = 0; ph < TS_L2_PH; ph++) {
        ...
        for (int32_t pw = 0; pw < TS_L2_PW; pw++) {
            const int32_t w0 = pw << 1;
            for (int32_t oc0 = oc_begin; oc0 < oc_end; ) {
                size_t vl = __riscv_vsetvl_e32m4((size_t)(oc_end - oc0));
                vint32m4_t vbias = __riscv_vle32_v_i32m4(bq + oc0, vl);
                vint32m4_t vacc00 = vbias;
                vint32m4_t vacc01 = vbias;
                vint32m4_t vacc10 = vbias;
...
#if TINYSPEECH_INT8_USE_VSE8_PACK_STORE
#if (TINYSPEECH_RVV_NCLIP_API == 4)
                vint16m2_t vq16 = __riscv_vnclip_wx_i16m2(vq, 0, __RISCV_VXRM_RNU, vl);
                vint8m1_t vq8 = __riscv_vnclip_wx_i8m1(vq16, 0, __RISCV_VXRM_RNU, vl);
#elif (TINYSPEECH_RVV_NCLIP_API == 3)
                vint16m2_t vq16 = __riscv_vnclip_wx_i16m2(vq, 0, vl);
                vint8m1_t vq8 = __riscv_vnclip_wx_i8m1(vq16, 0, vl);
#else
#error "TINYSPEECH_INT8_USE_VSE8_PACK_STORE requires valid TINYSPEECH_RVV_NCLIP_API"
#endif
...
#if TINYSPEECH_INT8_MULTICORE && defined(__riscv_vector) && TINYSPEECH_INT8_RVV_UKERNELS
typedef struct {
    const int8_t *pad;
    const int16_t *w16;
    const int32_t *bq;
...
static void ts_mc_worker_conv2(void *arg) {
    const ts_conv2_mc_job_t *job = (const ts_conv2_mc_job_t *)arg;
    conv3x3_pool2x2_requant_relu_to_padded_hwc_c_rvv_uk48x24(
        job->pad, job->w16, job->bq, job->mul_q31, job->dst_pad_hwc,
        job->oc_begin, job->oc_end, 0);
}

static void ts_mc_worker_conv3(void *arg) {
    const ts_conv3_mc_job_t *job = (const ts_conv3_mc_job_t *)arg;
    conv3x3_relu_gap_acc_hwc_padded_c_rvv_uk96x48(
        job->pad_hwc, job->w16, job->bq, job->gap_sum, job->oc_begin, job->oc_end);
}

static void ts_requant_gap_range(int32_t *gap_sum,
                                 int8_t *act_out,
                                 uint32_t mul_q31,
                                 int32_t oc_begin,
                                 int32_t oc_end) {
    for (int32_t oc = oc_begin; oc < oc_end; oc++) {
        int32_t avg_acc = (gap_sum[oc] + (TS_GAP_AREA / 2)) / TS_GAP_AREA;
        act_out[oc] = requant_u7_from_acc(avg_acc, mul_q31);
    }
}

static void ts_mc_worker_requant(void *arg) {
    const ts_requant_mc_job_t *job = (const ts_requant_mc_job_t *)arg;
    ts_requant_gap_range(job->gap_sum, job->act_out, job->mul_q31, job->oc_begin, job->oc_end);
}
#endif
