...
uint64_t target_frequency = TINYSPEECH_SC_TARGET_FREQUENCY_HZ;
...
static int run_suite_for_frequency(uint64_t frequency_hz) {
    ...
    printf("=== Bearly25 TinySpeech-SC @ %llu Hz ===\n", (unsigned long long)frequency_hz);
    printf("  mode: single-core inference benchmark\n");
#if TINYSPEECH_INT8_PIPELINE
...
#if TINYSPEECH_SC_ENABLE_PLL_SWEEP
static const uint64_t k_pll_sweep_freqs_hz[] = {
    TINYSPEECH_SC_PLL_FREQ_LIST
};
#endif
...
int main(void) {
    ...
    for (size_t i = 1; i < num_freqs; ++i) {
        ...
        reconfigure_pll(target_frequency, TINYSPEECH_SC_PLL_SWEEP_SLEEP_MS);
        status |= run_suite_for_frequency(target_frequency);
    }
    return status;
#else
