#ifndef BEARLY_HELLO_WFI_CONFIG_H
#define BEARLY_HELLO_WFI_CONFIG_H
...
/* How high Bearly counts locally before handing the baton off to DSP (baton = count + 1 = 11). */
#ifndef HELLO_WFI_LOCAL_COUNT
#define HELLO_WFI_LOCAL_COUNT 10u
#endif

/* ---- Cross-link address map (BML side) --------------------------------------------------------
 * BML's local scratchpad is 0xD000_0000. Its peer is DSP, whose spad is 0xC000_0000, reached over
 * the link by prepending a leading 1 -> 0x1_C000_0000. The peer's CLINT MSIP is reached the same
 * way (own MSIP 0x0200_0000 -> 0x1_0200_0000). */

/* Own scratchpad (we local-read the baton here). */
#ifndef HELLO_WFI_LOCAL_SPAD_BASE
#define HELLO_WFI_LOCAL_SPAD_BASE 0xD0000000ULL
#endif

/* Peer (DSP) scratchpad across the link (0xC000_0000 with a leading 1). */
#ifndef HELLO_WFI_PEER_SPAD_BASE
#define HELLO_WFI_PEER_SPAD_BASE 0x1C0000000ULL
#endif
 ...
 * OWN spad to decide whether a wake is really for it. This chip is Bearly, so its turn value is 1. */
#ifndef HELLO_WFI_MY_TURN
#define HELLO_WFI_MY_TURN 1u   /* Bearly */
#endif
#ifndef HELLO_WFI_PEER_TURN
#define HELLO_WFI_PEER_TURN 0u /* DSP */
#endif
...
#endif /* BEARLY_HELLO_WFI_CONFIG_H */
