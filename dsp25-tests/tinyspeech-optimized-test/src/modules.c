...
#if defined(__riscv_vector)
static void conv2d_rvv_direct_impl(const Tensor *input,
                                   const Tensor *weights,
                                   const Tensor *bias,
                                   float out_scale,
                                   Tensor *output) {
    ...
    for (int32_t n = 0; n < batch_size; n++) {
        ...
        for (int32_t oc = 0; oc < out_channels; oc++) {
            const float bias_v = tensor_get_channel_value(bias, oc);
            const float *w_oc = weights->f_data + oc * (in_channels * 9);
            float *out_oc = out_n + oc * out_hw;

            for (int32_t oh = 0; oh < out_height; oh++) {
                float *out_row = out_oc + oh * out_width;

                int32_t ow0 = 0;
                while (ow0 < out_width) {
                    size_t vl = __riscv_vsetvl_e32m4((size_t)(out_width - ow0));
                    vfloat32m4_t vbias = __riscv_vfmv_v_f_f32m4(bias_v, vl);
                    __riscv_vse32_v_f32m4(out_row + ow0, vbias, vl);
                    ow0 += (int32_t)vl;
                }
                ...
                for (int32_t ic = 0; ic < in_channels; ic++) {
                    ...
                    for (int32_t kh = 0; kh < 3; kh++) {
                        const int32_t ih = oh + kh - 1;
                        if ((ih < 0) || (ih >= in_height)) {
                            continue;
                        }
                        const float *in_row = in_c + ih * in_width;

                        for (int32_t kw = 0; kw < 3; kw++) {
                            const float wk = w_ic[kh * 3 + kw];
                            if (wk == 0.0f) {
                                continue;
                            }

                            int32_t ow_start = 1 - kw;
                            if (ow_start < 0) {
                                ow_start = 0;
                            }
                            int32_t ow_end = in_width + 1 - kw;
                            if (ow_end > out_width) {
                                ow_end = out_width;
                            }
                            if (ow_start >= ow_end) {
                            ...
                            const float *in_ptr = in_row + (ow_start + kw - 1);
                            float *out_ptr = out_row + ow_start;
                            int32_t len = ow_end - ow_start;
                            int32_t p = 0;
                            while (p < len) {
                                size_t vl = __riscv_vsetvl_e32m4((size_t)(len - p));
                                vfloat32m4_t vout = __riscv_vle32_v_f32m4(out_ptr + p, vl);
                                vfloat32m4_t vin = __riscv_vle32_v_f32m4(in_ptr + p, vl);
                                vout = __riscv_vfmacc_vf_f32m4(vout, wk, vin, vl);
                                __riscv_vse32_v_f32m4(out_ptr + p, vout, vl);
                                p += (int32_t)vl;
                            }
                        }
                    }
                ...
                if (inv_scale != 1.0f) {
                    int32_t p = 0;
                    while (p < out_width) {
                        size_t vl = __riscv_vsetvl_e32m4((size_t)(out_width - p));
                        vfloat32m4_t vout = __riscv_vle32_v_f32m4(out_row + p, vl);
                        vout = __riscv_vfmul_vf_f32m4(vout, inv_scale, vl);
                        __riscv_vse32_v_f32m4(out_row + p, vout, vl);
                        p += (int32_t)vl;
                    }
                }
            }
...
#if defined(__riscv_vector)
    if ((input->f_data != NULL) && (weights->f_data != NULL) && (stride == 1) &&
        (padding == 1) && (kernel_height == 3) && (kernel_width == 3)) {
        conv2d_rvv_direct_impl(input, weights, bias, out_scale, &output);
    } else {
