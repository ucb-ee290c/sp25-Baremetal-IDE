...
int app_main(void) {
    ...
    printf("=== DSP25 TinySpeech Optimized4 Test (INT8 pipeline + RVV fallback kernels) ===\n");
#if TINYSPEECH_INT8_PIPELINE
    ...
    printf("  int8 pack: vnclip+vse8 enabled (contiguous)\n");
#else
    printf("  int8 pack: scalar lane store fallback\n");
#endif
#elif defined(__riscv_vector)
