...
void tinyspeech_prepare_runtime(void) {
    ...
    printf("    prep: int8 weights prepack begin\n");
    fflush(stdout);
    if (tinyspeech_int8_prepare(W(0), W(3), W(6), W(9))) {
        printf("    prep: int8 weights prepack done\n");
    } else {
        printf("    prep: int8 weights prepack failed (fallback to float path)\n");
    }
    fflush(stdout);
#endif
}
...
Tensor tinyspeech_run_inference(Tensor *input) {
    ...
    if (tinyspeech_int8_is_ready()) {
        Tensor logits = tinyspeech_run_inference_int8(input, W(1), W(4), W(7), &g_last_cycle_profile);
        trace_store_logits(&logits);
        TRACE_ADD("fc_logits", &logits);
#if TINYSPEECH_OUTPUT_SOFTMAX
        uint64_t t0s = rdcycle64_model();
        softmax(&logits);
        uint64_t t1s = rdcycle64_model();
        g_last_cycle_profile.softmax = t1s - t0s;
        TRACE_ADD("softmax", &logits);
#endif
